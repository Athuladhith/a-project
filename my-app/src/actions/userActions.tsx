...
import { FETCH_CATEGORIES_REQUEST ,
  FETCH_CATEGORIES_FAIL,FETCH_CATEGORIES_SUCCESS
...
export const fetchRestaurants = (keyword?: string) => async (dispatch: Dispatch<AnyAction>) => {
  ...
  try {
      ...
      const { data } = await api.get('http://localhost:5000/api/users/category');
      console.log(data,"dd006")
...
export const passwordforgot=(userdata:{email:string})=>async (dispatch: Dispatch<AnyAction>) => {
  try {
    const response=await api.post('http://localhost:5000/api/users/forgotpassword',{email:userdata})
