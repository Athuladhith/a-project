...
//       // Destructure orderData or handle directly for COD
//       const { user, address, foodItems, totalAmount, paymentMethod } = orderData || req.body;
...
//       // Create delivery object from available details
//       const delivery = {
//           user,
//           address,
//           totalAmount,
...
//               totalAmount,
//               paymentMethod,
//               delivery, // Include the delivery object
//           });

//           return res.status(201).json({
//               success: true,
//               orderId: newOrder._id, // Use the MongoDB generated ID
//               amount: totalAmount,
//               currency: 'INR',
//               delivery, // Include the delivery object in the response
//           });
//       }
...
export const createConversation = async (req: Request, res: Response): Promise<Response> => {
  ...
  try {
    ...
    // Return the saved conversation in the response
    return res.status(201).json({
      success: true,
    ...
    return res.status(500).json({
      success: false,
      message: 'Server error, could not create conversation',
    });
  }
};


export const checkConversationExistence = async (req: Request, res: Response): Promise<Response> => {
  const { restaurantId, userId } = req.params;
  console.log(restaurantId,"reid 006")
  console.log(userId,"iuid0006")

  try {
    // Find if there is an existing conversation between the user and the restaurant
    const conversation = await Conversation.findOne({
      restaurantId: restaurantId,
      userId: userId,
    });
console.log(conversation,"found one 003")
    if (conversation) {
...
export const getFoodItemByIddd = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const foodItem = await Food.findOne({name:id});
...
export const getCategory = async (req: Request, res: Response): Promise<void> => {
  try {
    console.log("get in categories 0000000000333333333")
      const categories = await Category.find({});
      console.log(categories,"yess")
      res.json(categories);
